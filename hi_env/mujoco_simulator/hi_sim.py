...
class Simulator:
    def __init__(self, model_dir: Optional[str] = None):
        ...
        if model_dir is None:
            model_dir = os.path.join(os.path.dirname(__file__) + "/model/hi_cl_21dof")
        self.model_dir = model_dir

        # Load the model and data
        self.model: mujoco.MjModel = mujoco.MjModel.from_xml_path(f"{model_dir}/hi_cl_21dof.xml")
    ...
    def get_T_world_fbase(self) -> np.ndarray:
        ...
        """
        data = self.data.joint("torso_joint").qpos
        quat = data[3:]
        pos = data[:3]
    ...
    def set_T_world_fbase(self, T: np.ndarray) -> None:
        ...
        """
        joint = self.data.joint("torso_joint")
