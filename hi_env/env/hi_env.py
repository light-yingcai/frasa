...
from hi_env.mujoco_simulator.hi_sim import Simulator, tf


class HiEnv(gymnasium.Env):
    ...
    def __init__(self, render_mode="none", options: Optional[dict] = None, evaluation: bool = False):
        ...
        self.left_actuators_indexes = [self.sim.get_actuator_index(f"l_{dof}") for dof in self.dofs]
    ...
    def apply_control(self, q: np.ndarray, reset: bool = False) -> None:
        ...
        if reset:
            for k, dof in enumerate(self.dofs):
                self.sim.set_control(f"l_{dof}", q[k], True)
                self.sim.set_control(f"r_{dof}", q[k], True)
    ...
    def get_observation(self) -> np.ndarray:
        ...
        # Current control
        ctrl = [self.sim.get_control(f"l_{dof}") for dof in self.dofs]
        ctrl = np.array(ctrl)
    ...
    def step(self, action):
        ...
        if self.options["control"] == "position":
            ...
            start_q = [self.sim.get_q(f"l_{dof}") for dof in self.dofs]
        ...
        for k in range(timesteps):
            ...
            q = [self.sim.get_q(f"l_{dof}") for dof in self.dofs]
    ...
    def reset(
        ...
        # self.sim.set_control("l_shoulder_roll", -np.deg2rad(5))
        # self.sim.set_control("r_shoulder_roll", np.deg2rad(5))
